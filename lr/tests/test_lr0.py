...
def test_lr0_lr0(grammar_and_inputs: GrammarAndInputs) -> None:
...
def test_lr0_slr(grammar_and_inputs: GrammarAndInputs) -> None:
    grammar = grammar_and_inputs.grammar
    with pytest.raises(LoweringError):
        automaton = compute_automaton(grammar)

@parm_tests(grammar_examples.lalr)
def test_lr0_lalr(grammar_and_inputs: GrammarAndInputs) -> None:
    grammar = grammar_and_inputs.grammar
    with pytest.raises(LoweringError):
        automaton = compute_automaton(grammar)

@parm_tests(grammar_examples.lr1)
def test_lr0_lr1(grammar_and_inputs: GrammarAndInputs) -> None:
    grammar = grammar_and_inputs.grammar
    with pytest.raises(LoweringError):
        automaton = compute_automaton(grammar)

@parm_tests(grammar_examples.lr2)
def test_lr0_lr2(grammar_and_inputs: GrammarAndInputs) -> None:
    grammar = grammar_and_inputs.grammar
    with pytest.raises(LoweringError):
        automaton = compute_automaton(grammar)

@parm_tests(grammar_examples.ambiguous)
def test_lr0_ambiguous(grammar_and_inputs: Grammar) -> None:
